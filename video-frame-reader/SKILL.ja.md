...
description: MP4/GIF/MOVの動画ファイルからキーフレームを抽出して1枚のストリップ画像にまとめ、Claudeが分析できる形式にする。ユーザーが動画ファイルを共有し、アニメーション動作・UI遷移・スクショでは伝えにくい視覚的バグを分析したいときに使う。
license: MIT
---
...
```
/video-frame-reader [動画パス] [--format jpeg|png] [--threshold FLOAT] [--max-frames N]
/vfr [動画パス] [--format jpeg|png] [--threshold FLOAT] [--max-frames N]
```
...
| `--threshold` | `1.0` | 10階調を超えて変化したピクセルの割合（%）。小さいほど多く抽出 |
| `--max-frames` | `20` | 出力する最大キーフレーム数。超過分は均等に間引く |

## Claudeへの指示

### Step 0 — ファイルパスが省略されていたらfdで検索
...
```bash
vfr-extract <動画パス> [--format jpeg] [--threshold 1.0] [--max-frames 20]
```

`vfr-extract` が見つからない場合は、video-frame-readerプロジェクトのディレクトリで `uv tool install .` を実行するようユーザーに伝える。
